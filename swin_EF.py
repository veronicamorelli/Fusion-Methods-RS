...
class WindowAttention(nn.Module):
    ...
    Args:
        ...
        qkv_bias (bool, optional):  If True, add a learnable bias to query, key, value. Default: True 
            --> relative position bias added to self-attention computation
    ...
    def forward(self, x, mask=None):
        ...
        q = q * self.scale
        attn = (q @ k.transpose(-2, -1)) # 1600, 3, 49, 49

        relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1)].view(
            self.window_size[0] * self.window_size[1], self.window_size[0] * self.window_size[1], -1)  # Wh*Ww,Wh*Ww,nH
        relative_position_bias = relative_position_bias.permute(2, 0, 1).contiguous()  # nH, Wh*Ww, Wh*Ww
        attn = attn + relative_position_bias.unsqueeze(0) # 1600, 3, 49, 49
        ...
        attn = self.attn_drop(attn) # 1600, 3, 49, 49

        x = (attn @ v).transpose(1, 2).reshape(B_, N, C)
        x = self.proj(x) # 1600, 49, 96
        x = self.proj_drop(x) # 1600, 3, 49, 49
        return x
...
class SwinTransformerBlock(nn.Module):
    ...
    Args:
        ...
        window_size (int): Window size. (7 by default)
        shift_size (int): Shift size for SW-MSA.
        mlp_ratio (float): Ratio of mlp hidden dim to embedding dim.
        qkv_bias (bool, optional): If True, add a learnable bias to query, key, value. Default: True (relative positional bias)
        qk_scale (float | None, optional): Override default qk scale of head_dim ** -0.5 if set.
    ...
    def forward(self, x, mask_matrix):
        ...
        Args:
            x: Input feature, tensor size (B, H*W, C). ex: B, 4x4, 27
        ...
        shortcut = x # Shortcut: residual connection that will be added to the result of MLP(LN(x)) --> !!!!! Dubbio vedi foglio
        x = self.norm1(x)
        x = x.view(B, H, W, C)
        ...
        # partition windows
        x_windows = window_partition(shifted_x, self.window_size)  # nW*B, window_size, window_size, C   --> 1600, 7, 7, 96
        x_windows = x_windows.view(-1, self.window_size * self.window_size, C)  # nW*B, window_size*window_size, C ---> 1600, 49, 96

        ########################################################################################################

        attn_windows = self.attn(x_windows, mask=attn_mask)  # nW*B, window_size*window_size, C --> 1600, 49, 96

        ########################################################################################################

        # merge windows
        attn_windows = attn_windows.view(-1, self.window_size, self.window_size, C) # --> 1600, 7, 7, 96
        shifted_x = window_reverse(attn_windows, self.window_size, Hp, Wp)  # B H' W' C

        # reverse cyclic shift
        if self.shift_size > 0: # no
            x = torch.roll(shifted_x, shifts=(self.shift_size, self.shift_size), dims=(1, 2)) # no
        else:
            x = shifted_x # 16, 70, 70, 96

        if pad_r > 0 or pad_b > 0:
            x = x[:, :H, :W, :].contiguous() # 16, 64, 64, 96

        x = x.view(B, H * W, C) # 16, 4096, 96

        # FFN
        x = shortcut + self.drop_path(x) # add residual connection # 16, 4096, 96
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        x = x.view(B, H, W, C) # input è della dimensione dell'output del blocco transformer precedente
        ...
        x = self.norm(x)
        x = self.reduction(x) # questo fa l'effettiva riduzione 
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        mask_windows = mask_windows.view(-1, self.window_size * self.window_size) # 100, 49
        attn_mask = mask_windows.unsqueeze(1) - mask_windows.unsqueeze(2) # 100, 49, 49
        attn_mask = attn_mask.masked_fill(attn_mask != 0, float(-100.0)).masked_fill(attn_mask == 0, float(0.0)) # 100, 49, 49
        ...
        for blk in self.blocks: # blk blocks sono 4 blocchi di swin transformer (two successive swin transformer blocks)
            ...
            if self.use_checkpoint:
                ...
                x = blk(x, attn_mask) # transformer block
        
        ############################ SE VOGLIO SPLITTARE DEVO FARLO QUI ############################
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.hs_chans = hs_chans
        self.dem_chans = dem_chans
        ...
        self.patch_embed = DefaultPatchEmbed(
            patch_size=patch_size, rgb_chans=rgb_chans, hs_chans=hs_chans, dem_chans=dem_chans, embed_dim=embed_dim, #rgb_chans=rgb_chans, hs_chans=hs_chans, dem_chans=dem_chans
    ...
    def forward(self, x):
        ...
        x = self.patch_embed(x) # dim = 16, 96, 64, 64

        Wh, Ww = x.size(2), x.size(3) # 64, 64
        if self.ape: # ape=False di default
            # interpolate the position embedding to the corresponding size
            absolute_pos_embed = F.interpolate(self.absolute_pos_embed, size=(Wh, Ww), mode='bicubic')
            x = (x + absolute_pos_embed).flatten(2).transpose(1, 2)  # B Wh*Ww C
        else:
            x = x.flatten(2).transpose(1, 2) # dim = 16, 4096, 96
        x = self.pos_drop(x) # dim = 16, 4096, 96
    
        outs = []
        for i in range(self.num_layers):
            layer = self.layers[i] # i-esimo blocco di transformers
