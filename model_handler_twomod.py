...
class ModelHandler(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if batch_idx % 500 == 0:
            ...
            self.show_images(out_max, im1, im2, targets, step="im_train")
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx % 500 == 0:
            im1, im2 = samples
            out_max = torch.argmax(outputs, dim=1)
            self.show_images(out_max, im1, im2, targets, step="im_val")
    ...
    def test_step(self, batch, batch_idx):
        ...
        if batch_idx %2 == 1:
            im1, im2 = samples
            out_max = torch.argmax(outputs, dim=1)
            self.show_images(out_max, im1, im2, targets, step="im_test")
    ...
    def show_images(self, out, im1, im2, gt, step:str): 
        ...
        im1 = torch.clamp(im1, min=0, max=1)
        im2 = torch.clamp(im2, min=0, max=1)
        ...
        if self.modality == 'rgb_dem':
            self.logger.experiment.add_image(step+'/'+self.modality.split('_')[0], utils.make_grid(im1), self.global_step)
            self.logger.experiment.add_image(step+'/'+self.modality.split('_')[1], utils.make_grid(im2), self.global_step)
        elif self.modality == 'rgb_hs':
            self.logger.experiment.add_image(step+'/'+self.modality.split('_')[0], utils.make_grid(im1), self.global_step)
            self.logger.experiment.add_image(step+'/'+self.modality.split('_')[1], utils.make_grid(im2.mean(1).unsqueeze(1)), self.global_step)
        elif self.modality == 'hs_dem':
            self.logger.experiment.add_image(step+'/'+self.modality.split('_')[0], utils.make_grid(im1.mean(1).unsqueeze(1)), self.global_step)
            self.logger.experiment.add_image(step+'/'+self.modality.split('_')[1], utils.make_grid(im2), self.global_step)
        else: 
            print("modalities not accepted")
