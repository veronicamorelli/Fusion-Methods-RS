...
train_dataloader = DataLoader(train_set, batch_size=config["batch_size"], shuffle=True, num_workers=4)
val_dataloader = DataLoader(val_set, batch_size=config["batch_size"], shuffle=False, num_workers=4)
test_dataloader = DataLoader(test_set, batch_size=config["batch_size"], shuffle=False, num_workers=4)
...
# criterion = FocalLoss(ignore_index=0, gamma=1)
...
trainer = Trainer(#callbacks=[EarlyStopping(monitor="losses/val_loss", mode="min")],
...
# deterministic=True per togliere randomness dal nel training (meglio di quanto faccia il seed e basta), nonostante ció, potebbe rallentare il training
