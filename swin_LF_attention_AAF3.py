...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, H, W, i):
        ...
        for blk in self.blocks: # blk blocks sono 4 blocchi di swin transformer (two successive swin transformer blocks)
            ...
            mask_list.append(mask_for_list)
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            ...
            x_out, H, W, x, Wh, Ww, masks = layer(x, Wh, Ww, i) 
            ...
            if i in self.out_indices:
                ...
                hs_outs.append(hs_out2)

                # rgb_outs: 
                # type list
                # len 4
                # rgb_outs[0].shape = torch.Size([8, 96, 64, 64])
                # rgb_outs[1].shape = torch.Size([8, 192, 32, 32])
                # rgb_outs[2].shape = torch.Size([8, 384, 16, 16])
                # rgb_outs[3].shape = torch.Size([8, 768, 8, 8])
                
                # same for hs_outs
            
            for i in range(len(masks)): # for sulle mask (tante mask quanti sono i layer in 1 transformer block)
                mask_rgb = masks[i][0]
                mask_hs = masks[i][1]
                mask_list_rgb.append(mask_rgb)
                mask_list_hs.append(mask_hs)
                

        return [rgb_outs, hs_outs], [mask_list_rgb, mask_list_hs] # outs
        # outs is a list