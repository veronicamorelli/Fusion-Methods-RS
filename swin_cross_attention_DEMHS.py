...
class WindowCrossAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        q1 = q1 * self.scale
        attn1 = (q1 @ k2.transpose(-2, -1)) # 1600, 3, 49, 49
        ...
        if mask is not None:
            ...
            # mask = 100, 49, 49
            nW = mask.shape[0] # 100
            attn1 = attn1.view(B_ // nW, nW, self.num_heads, N, N) + mask.unsqueeze(1).unsqueeze(0) # 8, 100, 3, 49, 49
        ...
        x1 = (attn1 @ v2).transpose(1, 2).reshape(B_, N, C) # 1600, 49, 96
        x1 = self.proj1(x1) # 1600, 49, 96
        x1 = self.proj_drop1(x1)

        x2 = (attn2 @ v1).transpose(1, 2).reshape(B_, N, C) # 1600, 147, 96
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask_matrix):
        ...
        rgb, hs = x # 8, 4096, 96 each
        mask_matrix_rgb, _ = mask_matrix # 100, 49, 49 
        ...
        if self.shift_size > 0:
            ...
            attn_mask = mask_matrix # --> che dim ha? 100, 49, 49
        ...
        attn_windows_rgb, attn_windows_hs = attn_windows
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        attn_mask = attn_mask_rgb, attn_mask_hs
        x = rgb, hs
    ...
    def attn_mask_creation(self, x, H, W):
        ...
        Hp = int(np.ceil(H / self.window_size)) * self.window_size # 70
        Wp = int(np.ceil(W / self.window_size)) * self.window_size # 70
        img_mask = torch.zeros((1, Hp, Wp, 1), device=x.device)  # 1 70 70 1
        ...
        mask_windows = window_partition(img_mask, self.window_size)  # nW, window_size, window_size, 1 100, 7, 7, 1
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        rgb, hs, dem = x
        hs2 = torch.concat((hs, dem), dim=1)
        rgb = self.patch_embed_rgb(rgb) 
        hs = self.patch_embed_hs(hs2)
