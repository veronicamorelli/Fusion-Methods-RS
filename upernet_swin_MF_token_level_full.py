...
from swin_MF_token_level_full import SwinTransformer
...
class UperNet_swin(nn.Module):
    ...
    def forward(self, x):
        ...
        # len(features) = 4
        # features[0].shape = torch.Size([16, 288, 64, 64])
        # features[1].shape = torch.Size([16, 576, 32, 32])
        # features[2].shape = torch.Size([16, 1152, 16, 16])
        # features[3].shape = torch.Size([16, 2304, 8, 8])
        features[-1] = self.PPN(features[-1])
        x = self.head(self.FPN(features)) # 16, 8, 64, 64

        x = F.interpolate(x, size=input_size, mode='bilinear') # 16, 8, 256, 256
        return x
