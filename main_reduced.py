...
transform_composed_alb_train = A.Compose([A.Resize(height=config["input_size"][0], width=config["input_size"][1]),
                                          A.augmentations.crops.transforms.RandomCrop(config["train_size2"][0], config["train_size2"][1]),
                                          A.Rotate(limit = 90),
...
transform_composed_alb_val = A.Compose([A.Resize(height=config["train_size2"][0], width=config["train_size2"][1]), # é train_size giusto?
                                        ToTensorV2()],
                                          additional_targets={
                                            "hs": "image",
	                                          "dem": "image"
