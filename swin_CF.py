...
from Patch_embedding import ChannelPatchEmbed
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask_matrix):
        ...
        shortcut = x # Shortcut: residual connection that will be added to the result of MLP(LN(x))
        x = self.norm1(x)
        x = x.view(B, H, W, C)
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.embed_dim_after_CF = embed_dim_after_CF
        self.ape = ape
        ...
        for i_layer in range(self.num_layers): # num_layers=4 len(dephts)
            layer = BasicLayer(
                dim=int(embed_dim_after_CF * 2 ** i_layer),
        ...
        num_features = [int(embed_dim_after_CF * 2 ** i) for i in range(self.num_layers)]
    ...
    def forward(self, x):
        ...
        x = self.patch_embed(x) # dim = 16, 256, 64, 64
        ...
        x = self.pos_drop(x) # dim = 16, 4096, 384
